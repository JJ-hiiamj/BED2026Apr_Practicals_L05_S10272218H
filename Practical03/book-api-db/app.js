...
app.put("/books/:id", async (req, res) => {
  ...
  try {
    ...
    updateRequest.input("title", sql.VarChar, updatedBookData.title);
    updateRequest.input("author", sql.VarChar, updatedBookData.author);
    
    const result = await updateRequest.query(updateQuery);

    // 2. Check if the book existed (rowsAffected is an array, usually [1])
    if (result.rowsAffected[0] === 0) {
      return res.status(404).send("Book not found");
    }

    // 3. Fetch the updated data to return to the user
    const getUpdatedBookQuery = `SELECT id, title, author FROM Books WHERE id = @id`;
    const getRequest = connection.request();
    getRequest.input("id", sql.Int, bookId);
    const finalResult = await getRequest.query(getUpdatedBookQuery);
...
app.delete("/books/:id", async (req, res) => {
  ...
  try {
    ...
    // 204 No Content is the standard for successful deletion with no body
    res.status(204).send(); 
  } catch (error) {
